...
void cache_sim_t::init()
{
  ...
  accesstimes = new size_t[sets*ways];
  read_accesses = 0;
  read_misses = 0;
  bytes_read = 0;
  write_accesses = 0;
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      size_t way = (&tags[idx*ways + i] - &tags[0]) % ways;
      accesstimes[idx*ways + way] ++;
...
uint64_t cache_sim_t::victimize(uint64_t addr){
  ...
  for(size_t i = 0; i < ways; i ++){
	size_t accesses = accesstimes[idx*ways + i];
	if(accesses < min_access){
		min_access = accesses;
		lfu_way = i;
	}
  }
  
  uint64_t victim = tags[idx*ways + lfu_way];
  tags[idx*ways + lfu_way] = (addr >> idx_shift) | VALID;
  accesstimes[idx*ways + lfu_way] = 0;
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  if (tags.size() == ways)
  {
    ...
    auto it = tags.begin();
    std::advance(it, lfu_way);
    old_tag = it->second;
    tags.erase(it);
    accesstimes[lfu_way] = 0;
  }
