...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t lru_way = 0;
  size_t max_hittime = 0;
  
  for(size_t i = 0; i < ways; i ++){
     if(hittimes[idx*ways + i] > max_hittime){
     	max_hittime = hittimes[idx*ways + i];
     	lru_way = i;
     }
  }
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  {
    for (size_t i = 0; i < ways; i++) {
	if (hittimes[i] > max_hittime) {
		max_hittime = hittimes[i];
		lru_way = i;
	}	
    }
    auto it = tags.begin(); 
    std::advance(it, lru_way);
    old_tag = it->second;
    tags.erase(it);
    hittimes[lru_way] = 0;
  }
