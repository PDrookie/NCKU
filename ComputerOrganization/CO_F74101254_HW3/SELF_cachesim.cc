...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t mru_way = 0;
  uint64_t min_hittime = 1;
  
  for(size_t i = 0; i < ways; i ++){
     if(hittimes[idx*ways + i] == min_hittime){
     	mru_way = i;
     }
  }
  
  uint64_t victim = tags[idx*ways + mru_way];
  tags[idx*ways + mru_way] = (addr >> idx_shift) | VALID;
  hittimes[idx*ways + mru_way] = 1;
...
uint64_t fa_cache_sim_t::victimize(uint64_t addr)
{
  ...
  {
    for(size_t i = 0; i < ways; i ++){
        if(hittimes[i] == min_hittime){
     	    mru_way = i;
        } 
    }
    auto it = tags.begin();
    std::advance(it, mru_way);
    old_tag = it->second;
    tags.erase(it);
    hittimes[mru_way] = 1;
  }
